// PONSI (3,3) dashboard — metrics, live rebase, stake/bond/claim.
// All state is the off-chain ledger in server/index.js. Nothing here touches a contract.
(function () {
  const $ = (id) => document.getElementById(id);
  let M = null, A = null, wallet = localStorage.getItem('ponsi_w') || '';
  ...
  // ---- theme (PONS defaults light, remembers the choice) ----
  const themeBtn = $('themeBtn');
  function setTheme(t) {
    document.documentElement.setAttribute('data-theme', t);
    localStorage.setItem('ponsi_theme', t);
    if (themeBtn) themeBtn.setAttribute('aria-label', t === 'dark' ? 'Switch to light mode' : 'Switch to dark mode');
  }
  setTheme(localStorage.getItem('ponsi_theme') || 'light');
  ...
  function setWallet(a) {
    if (a && isW(a)) { wallet = a; localStorage.setItem('ponsi_w', a); renderWallet(); loadAccount(); }
    else { wallet = ''; A = null; localStorage.removeItem('ponsi_w'); renderWallet(); ['yStaked', 'yBalance', 'yNext'].forEach((id) => $(id).textContent = '—'); renderYourBonds(); }
  }
  ...
  function renderMetrics() {
    ...
    $('mBacking').textContent = 'backing $' + M.backingPerToken.toFixed(6) + ' / $PONSI';
    ...
    if (M.leaderboard && M.leaderboard.length) {
      ...
      $('lbRows').innerHTML = M.leaderboard.map((b, i) => `<div class="row"><span>${i + 1}. <b class="cd">${b.wallet}</b></span><span><b class="tl">${tok(b.staked)} sPONSI</b> <span style="color:var(--mut)">· ${pct(b.share)}</span></span></div>`).join('');
    }
    calc();
    if (M.mint) { const bar = $('ca'); bar.style.display = 'flex'; $('caV').textContent = M.mint.slice(0, 6) + '…' + M.mint.slice(-4); bar.href = 'https://pons.family/token/' + M.mint; $('caCopy').onclick = (e) => { e.preventDefault(); navigator.clipboard && navigator.clipboard.writeText(M.mint); $('caCopy').textContent = 'Copied'; setTimeout(() => $('caCopy').textContent = 'Copy', 1200); }; }
  }
  function renderAccount() {
    if (!A) return;
    $('yBalance').textContent = tok(A.balance) + ' $PONSI';
    $('yNext').textContent = '+' + (A.staked * M.rate).toFixed(4) + ' $PONSI';
  }
  ...
  function renderYourBonds() {
    ...
    $('yourBonds').innerHTML = A.bonds.map((b) => `
      <div class="yb"><span>${b.market} · <b>${tok(b.payout)}</b> $PONSI</span>
  ...
  async function doClaim(autostake) {
    ...
    if (r.error) return toast(r.error); A = r; reanchor(); renderAccount(); renderYourBonds(); toast(autostake ? 'Claimed & staked ' + tok(r.claimed) : 'Claimed ' + tok(r.claimed) + ' $PONSI');
  }
  ...
  function tick() {
    ...
    $('mStaked').textContent = tok(anchor.totalStaked * ratio) + ' sPONSI';
    if (A && anchor.agons) { $('yStaked').textContent = (anchor.agons * li.index).toFixed(4) + ' sPONSI'; }
    else if (A) $('yStaked').textContent = '0.0000 sPONSI';
  }
  // ---- calculator ----
  function calc() {
    ...
    $('calcOut').textContent = tok(out) + ' $PONSI';
