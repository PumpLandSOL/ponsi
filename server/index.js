// PONSI ($PONSI) — OHM-style (3,3) staking + bonding + treasury protocol.
// Launched on PONS (pons.family), which mints FIXED-SUPPLY ERC-20s on Robinhood Chain.
// Fixed supply => no mint authority => rebases cannot emit on-chain. The index, the
// sPONSI wrapper and the treasury are therefore an off-chain ledger: a simulation.
...
const TOKEN = process.env.TOKEN_TICKER || 'PONSI';
const DATA_PATH = process.env.DATA_PATH || path.join(ROOT, 'data.json');
const PONSI_MINT = process.env.PONSI_MINT || '0xDae574d95eA1bb30c94255a07e733EC62BE75dDF';  // $PONSI · launched on PONS
...
const BONDS = [
  ...
  { id: 'lp', name: 'PONSI-ETH LP', discount: 0.13, vestDays: 5 },
...
http.createServer(async (req, res) => {
  ...
  serve(req, res);
}).listen(PORT, () => console.log('PONSI ($' + TOKEN + ') on :' + PORT + ' — APY ' + APY_TARGET + '% (simulated), rebase ' + REBASE_SEC + 's'));
